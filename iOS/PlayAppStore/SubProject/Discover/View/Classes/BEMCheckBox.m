...
//加号
@property (strong, nonatomic) CAShapeLayer * plusSignLayer;
...
- (void)setMarklineWidth:(CGFloat)marklineWidth {
    ...
    _pathManager.marklineWidth = marklineWidth;
    [self reload];
}
...
- (void)drawplusSignLayer {
    [self.plusSignLayer removeFromSuperlayer];
    self.plusSignLayer = [CAShapeLayer layer];
    self.plusSignLayer.frame = self.bounds;
    self.plusSignLayer.path = [self.pathManager pathForPlusSign].CGPath;
    self.plusSignLayer.strokeColor = self.tintColor.CGColor;
    self.plusSignLayer.lineWidth = self.marklineWidth;
    self.plusSignLayer.fillColor = [UIColor clearColor].CGColor;
    self.plusSignLayer.lineCap = kCALineCapRound;
    self.plusSignLayer.lineJoin = kCALineJoinRound;
    
    self.plusSignLayer.rasterizationScale = 2.0 * [UIScreen mainScreen].scale;
    self.plusSignLayer.shouldRasterize = YES;
    [self.layer addSublayer:self.plusSignLayer];
...
- (void)drawCheckMark {
    ...
    self.checkMarkLayer.lineWidth = self.marklineWidth;
...
- (void)addOffAnimation {
    ...
    switch (self.offAnimationType) {
        ...
        case BEMAnimationTypeBounce: {
            ...
            [self.onBoxLayer addAnimation:opacity forKey:@"opacity"];
            [self.checkMarkLayer addAnimation:wiggle forKey:@"transform"];
            [self performSelector:@selector(afterMarkRemoved) withObject:nil afterDelay:wiggle.duration];
        }
            return;
...
- (void)afterMarkRemoved {
    ...
    opacity.duration = self.animationDuration / 1.4;
    [self drawplusSignLayer];
    [self.plusSignLayer addAnimation:wiggle forKey:@"transform"];
